...
\section{Associate Editor Review}
    ...
    \subsection{Non-negotiable technical corrections}
        ...
        We inserted a short snippet in the \hl{Section 2.3} to explain how we computed the metrics in the multiclass setting to ensure transparency and reproducibility of the evaluation protocol. We also showed the metrics for all classes in the supplementary material.
    ...
    \subsection{There are also a few minor issues:}
        ...
        \hl{Fernando, pensei em criar uma nova métrica que relaciona $\Delta NO$ com $\Delta H$, tipo um quociente de variação (derivada) para tentar capturar melhor o impacto na tarefa downstream. O que você acha?}

        \subsection{Minor Issue \#3}
