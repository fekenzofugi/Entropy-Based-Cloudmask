...
\AND
Marlon F. de Souza \\
Group of Research and Extension in Agroindustrial Logistics -- ESALQ-LOG \\
...
\maketitle
    ...
    \section{Introduction}
        ...
        There has been increasing research interest in algorithms for effective cloud screening in RS imagery \citep{aybar2024cloudsen12plus}. Numerous CM algorithms have been proposed, including those by \citet{zhu2012object,foga2017cloud,zhu2018automatic,pasquarella2023comprehensive} and \citet{francis2024sensor}. Researchers have proposed a wide range of CM techniques, including physical rule-based methods and advanced machine-learning approaches, each demonstrating varying levels of accuracy across different geographic regions and cloud conditions. Notably, in many cases, the presence of thin clouds, despite disturbing the image, still allows land cover (LC) classifiers to identify target objects correctly \citep{gawlikowski2022explaining,wang2024empirical}.

        A large number of CM algorithms for optical satellite imagery exist, but few studies have conducted systematic intercomparisons of them \citep{skakun2022cloud}. While most research has evaluated CMs with traditional metrics, such as overall accuracy (OA) and F1-score, there is a notable gap in measuring how these algorithms affect the outcomes of subsequent classification or segmentation models. Moreover, no prior work has explored entropy-based methods to determine the effect of decreasing uncertainty while retaining most information. Therefore, this paper aims to compare seven CM algorithms based on OA, F1, Intersection over Union (IoU), Delta Entropy \(\Delta H\), and the variation in the results of a robust AI model caused by CM \(\Delta NO\), using the CloudSEN12+ manually labeled reference mask as ground truth.
        ...
        We introduced $\Delta H$ to measure the impact of CM on information retention. Additionally, we evaluated how this essential RS preprocessing step influences the performance of robust machine learning (ML) models, comparing the number of objects segmented in raw versus cloud-masked images to quantify the change in segmentation due to masking. The CM effectively minimized perturbations caused by cloud cover, resulting in SAM achieving a higher number of segmented objects. The rest of this paper is organized into four main sections, following this introduction. Section 2 provides a concise review of the theoretical background that motivated our exploration of entropy. Section 3 describes the methodology. Results and discussion are in Section 4. Finally, Section 5 summarizes our conclusions.
    ...
    \section{Entropy in the Context of Digital Images}
        ...
        \subsection{Evaluation Metrics}
            ...
            \begin{equation}
            \label{eq 9}
                \Delta{H(R)}=H(R)_{Raw}-H(R)_{Masked}
            \end{equation}

            If $\Delta H(R)>0$, this implies there was an increase in the scene uncertainty. Inversely, $\Delta H(R)<0$ means that the CM decreased the uncertainty, increasing the image predictability. $\Delta H(R) \approx 0$ indicates that the CM did not change the information content. In order to compare and evaluate the CMs, we calculated the mean of $\Delta H$ in each band across the image set and named it $\overline{H(\text{band})}$. Given that the RGB bands exhibit similar values and behavior (See Figure 8 in Section 4), the red band was chosen as the only one shown in some analyses. For simplicity, we adopted $\overline{\Delta{H}}$ to refer to $\overline{\Delta H(R)}$ in the rest of the paper. 
        ...
        \label{results_3}
            ...
            In a search for S2 scenes where CM had a minor impact on segmentation outcomes, we found some examples, as the one presented in Figure 12. This scene features a complex landscape with unclear object boundaries, irregular shapes, low contrast, and hard-to-distinguish textures (Figure 12.a). Segmenting smaller and irregular objects poses difficulties for SAM, regardless of whether we use automatic or prompt-based segmentation \citep{osco2023segment}. Figure 12b illustrates the situation where SAM struggled to define the objects. In such instances, cloud masking has a minor impact on the segmentation results (Figure 12c). Entropy alone does not fully define the predictability of an image. Combining spectral, textural, and contextual features with entropy would be necessary regardless of whether the image is a photomicrograph or a satellite image. For example, to accurately assess the unpredictability of the texture, we first need to create the co-occurrence matrix for all objects in the image and then calculate entropy from this matrix to quantify the unpredictability of the texture \citep{haralick1973textural}.
