...
\maketitle
    ...
    \section{Introduction}
        ...
        \hl{Why is it pertinent to evaluate cloud masks using metrics from downstream tasks? Cloud masking is not an end in itself. Instead, it emerged as a preprocessing step in remote sensing endeavors to reduce noise in satellite imagery and to enhance the outcomes of subsequent tasks. Therefore, assessing cloud masks based on their impact on downstream tasks offers a more practical, application-oriented perspective. This approach allows us to evaluate how effectively cloud masks improve data quality for specific applications, such as land cover classification, change detection, and image segmentation. By focusing on the end goals of remote sensing projects, we can better understand the utility of different cloud masking techniques and select the most appropriate method for each use case.}

        \hl{When we relate $\overline{\Delta NO}$ and $\overline{\Delta H}$, we were not assessing the number of objects (NO) detected by the downstream segmenter or the data entropy. Rather, we used the concept of the derivative to assess the rate of change in NO resulting from uncertainty variation induced by the CM. We hypothesize that a decrease in entropy leads to an increase in the NO detected by SAM. This relationship suggests that, as the image becomes more predictable and less uncertain through effective cloud masking, SAM can identify and segment more distinct objects in the scene. However, if entropy does not decrease or decreases excessively, leading to fewer objects being detected, it indicates that the cloud masking was less effective. This analysis provides insights into how cloud masking influences segmentation quality by modulating the underlying uncertainty in the imagery.}
